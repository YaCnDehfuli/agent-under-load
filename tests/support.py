...
def write_rule(directory: Path, body: str = RULE_YAML,
    ...
    return corpus.RuleRef(id="abc", title="Suspicious LSASS Access", level="high",
...
def synthetic_triage_case(rule: corpus.RuleRef) -> corpus.TriageCase:
    return corpus.TriageCase(
        ...
        rule=rule,
        capture=corpus.CaptureRef(id="lab/synthetic", group="atomic",
                                  archive=Path("nowhere.zip"), events=3),
        fire_count=1,
        truth="true_positive",
    )
